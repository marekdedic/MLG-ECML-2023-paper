\documentclass[sn-mathphys,pdflatex,iicol]{sn-jnl}

%\usepackage{polyglossia} % Must come before biblatex
\usepackage{adjustbox}
%\usepackage{amsfonts}
%\usepackage{amsmath}
%\usepackage{bm}
%\usepackage{booktabs}
\usepackage{csquotes}
%\usepackage[T1]{fontenc}
%\usepackage{fontspec}
\usepackage{geometry}
%\usepackage{graphicx}
%\usepackage{hyperref}
%\usepackage{lua-visual-debug}
%\usepackage{tabularx}

%\setdefaultlanguage{english}
%\setotherlanguage{czech}

\jyear{2023}
\raggedbottom

\hypersetup{
	pdfencoding=auto,
	unicode=true,
	bookmarksopen=true,
	bookmarksopenlevel=3
}

\geometry{margin=1cm, inner=0.5cm, bottom=0.7cm}
\makeatletter
\def\sectionfont{\reset@font\fontfamily{\rmdefault}\fontsize{11bp}{12bp}\bfseries\selectfont\raggedright\boldmath}
\makeatother

\newcommand{\name}[1]{\textit{#1}}
\newcommand{\mathfield}{\ensuremath{\mathbb}}
\newcommand{\mathmat}{\ensuremath{\mathbf}}
\newcommand{\mathset}{\ensuremath{\mathbb}}
\newcommand{\mathspace}{\ensuremath{\mathcal}}
\newcommand{\mathvec}{\ensuremath{\bm}}

\newcounter{enumroman}
\newenvironment{romanitems}{\begin{list}{\bfseries(\roman{enumroman})\hfill}{\usecounter{enumroman}\setlength{\labelwidth}{\leftmargin}\addtolength{\labelwidth}{-1\labelsep}\topsep=0mm plus 2pt\itemsep=0mm\parsep=0mm plus 2pt\itemindent=0mm}}{\end{list}}

\DeclareMathOperator*{\argmin}{arg\,min}
\DeclareMathOperator*{\argmax}{arg\,max}
