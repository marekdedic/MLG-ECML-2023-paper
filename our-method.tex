...
\For{g = 1,\dots,M}
\State Offspring $\gets$ \textsc{Crossover}(Pop, $p_\mathrm{cx}$) \Comment{Partitions Pop into pairs and applies crossover to each pair with probability $p_\mathrm{cx}$} \label{eva:crossover}
\State Offspring $\gets$ \textsc{Mutate}(Offspring, $p_\mathrm{gene}$, $p_\mathrm{mut}$) \Comment{Apply mutation to each individual with probability $p_\mathrm{mut}$, mutating each gene with probability $p_\mathrm{gene}$} \label{eva:mutate}
\State $\mathbf{f} \gets$ \textsc{Evaluate}(Offspring, $G$, $\Theta$) \label{eva:eval} \Comment{Map $f$ to Offspring}
\State Pop $\gets$ \textsc{Select-Tournament}(Offspring, $\mathbf{f}$, $n_\mathrm{tourn}$) \label{eva:select} \Comment{Replace the parent population}
