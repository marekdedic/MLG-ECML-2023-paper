...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \Function{get\_node\_order}{$ G, \bm{e}_{i+1}, \bm{y}_\mathrm{train}, replacement\_maps $}
        ...
        \State \Return $ \Call{sort}{V \left( G \right), \mathrm{by} = entropy\_per\_node, \mathrm{descending} = \mathrm{true}} $
    \EndFunction
    \Statex
    \Function{select\_nodes}{$ ordered\_nodes, num\_nodes\_to\_prolong, replacement\_maps $}
        \State $ count \gets 0 $
        \State $ selected\_merges \gets \left\{ \right\} $
        \For{$ node \in ordered\_nodes $}
            \State $ merge \gets \Call{resolve\_merge}{node, selected\_merges, replacement\_maps} $
            \If{$ merge = \mathrm{null} $}
                \State continue
            \EndIf
            \State $ \Call{push}{selected\_merges, merge} $
            \State $ count \gets count + 1 $
            \If{$ count >= num\_nodes\_to\_prolong $}
                \State break
            \EndIf
        \EndFor
    ...
    \Function{resolve\_merge}{$ node, already\_selected\_merges, replacement\_maps $}
        ...
        \For{$ replacement\_map \in replacement\_maps $} \Comment Iterate over replacements from finest to coarsest graph
            \State $ merge\_candidate \gets \Call{find\_merge}{node} $ \Comment Find a merge affecting the node
            \If{$ merge\_candidate = \mathrm{null} $}
                \State continue
            \EndIf
