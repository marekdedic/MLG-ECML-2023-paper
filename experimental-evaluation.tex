...
The values of these metrics at different steps of the adaptive prolongation algorithm are shown in Figure~\ref{fig:metrics-absolute} and Figure~\ref{fig:metrics-incremental}. On the Twitch dataset, the metrics show very few consistent behaviours, namely the universal rice in node homophily in the absolute scenario and the sudden rise of label informativeness in the incremental scenario. The behaviour on the other datasets is substantially different with a general preference for a slow rise in the metric value in the absolute scenario, with the exception of global assortativity. As for PubMed and DBLP specifically, most of the metrics rise quickly and then plateau at the same point as the local minimum of the performance. This suggests the explanation of the graph being heterophilic when very coarse, then reaching a point where new nodes are mostly added to existing neighbourhoods, at which point the accuracy temporarily stops growing.
